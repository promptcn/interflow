...
pub fn build_root_store(ca_path: Option<&str>) -> Result<rustls::RootCertStore> {
...
pub fn load_pem_cert_chain(path: &str) -> Result<Vec<rustls::pki_types::CertificateDer<'static>>> {
...
pub fn load_pem_private_key(path: &str) -> Result<rustls::pki_types::PrivateKeyDer<'static>> {
    let file = File::open(path)
...
pub fn build_client_config(
    ...
    let builder = if let Some(pin) = pin_hex {
        let verifier = crate::tls::cert_pin::make_pinned_verifier(pin)
    ...
    let mut config = match (client_cert_path, client_key_path) {
        (Some(cert_path), Some(key_path)) => builder
            ...
            .map_err(|e| InterflowError::config(format!("client certificate setup failed: {e}")))?,
