...
fn main() {
    ...
    //   - .git/logs/HEAD: catches new commits on the same branch (while HEAD is
    //     a symbolic ref, .git/HEAD itself is not rewritten, so only the
    //     reflog can observe new commits)
